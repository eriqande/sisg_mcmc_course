 ...
 ) 
{
	...
	if( FinalPass==1) {
		...
		if( *how_many_times == 0 && IsRequired==2 && CoDependent>0 ) {
			option_name(tempStr, (char *)sh, (char *)lon);
			fprintf(stderr,"Error Processing Option %s!    The option %s is absent, but is required when %s\n",tempStr, tempStr, CondReqOptString);
			*Error = 1;
		}
...
void insert_file_into_commline(char ***argv, int *argc, int *i, char *FileName)
{
	...
	*i = *i - 2;
	...
	if(N==0 && start == *argc-1) {
		fprintf(stderr,"Error Processing Option --command-file! The last option you gave was for inserting a command file that appears to be empty.  Illegal\n");
		exit(1);
	}
